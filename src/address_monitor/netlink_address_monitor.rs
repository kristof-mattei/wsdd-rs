...
impl NetlinkAddressMonitor {
    ...
    pub async fn process_changes(&self) -> Result<(), eyre::Report> {
        ...
        // Notice the buffer 4 byte aligned buffer because all of our structs written here by the kernel
        ...
        loop {
            ...
            let buffer = &raw const buffer.as_ref()[..bytes_read] as *const [u8];
...
async fn parse_netlink_response(
    ...
    let mut raw_nlh = SendPtr::new(buffer, buffer.as_ptr().cast::<nlmsghdr>());
