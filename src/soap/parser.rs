...
fn parse_probe<'raw>(reader: &mut NsReader<&'raw [u8]>) -> ParsedProbe<'raw> {
    let mut types = None;
        ...
        match reader.read_resolved_event()? {
            (Bound(Namespace(ns)), Event::Start(e)) => {
                if ns == XML_WSD_NAMESPACE.as_bytes() && e.name().local_name().as_ref() == b"Scopes"
                {
                    ...
                    // TOOD error
                    return Err(color_eyre::Report::msg("Scopes unsupported"));
    ...
    let Some(types) = types else {
        ...
        // TODO error
        return Err(eyre::Report::msg("Missing types"));
    };
    ...
    if types.trim() != WSD_TYPE_DEVICE {
        ...
        return Err(eyre::Report::msg("unknown discovery type for probe"));
    }
...
pub fn parse_probe_body<'raw>(reader: &mut NsReader<&'raw [u8]>) -> ParsedProbe<'raw> {
    loop {
        match reader.read_resolved_event()? {
            (Bound(Namespace(ns)), Event::Start(e)) => {
    ...
    // TODO error
    Err(eyre::Report::msg("Missing ./Probe in body"))
}

fn parse_resolve<'raw>(reader: &mut NsReader<&'raw [u8]>, target_uuid: Uuid) -> ParsedProbe<'raw> {
    let mut addr = None;
...
pub fn parse_resolve_body<'raw>(
    ...
    // TODO error
    Err(eyre::Report::msg("Missing ./Resolve in body"))
}
